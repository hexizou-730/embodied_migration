"""Failing seed adapter for abstract-hint xarm6 LLM generation.

This seed is the same kind of non-oracle waypoint controller used in case03.
The difference is in the prompt: case04 gives the LLM an abstract contact-side
strategy, but still withholds the concrete oracle action values and step counts.
"""
...
class GeneratedXArm6AbstractHintPullCubeRobot(ManiSkillPullCubeRobot):
    """Non-oracle seed adapter used for abstract-hint LLM generation."""
    ...
    def _validate_action_space(self) -> None:
        ...
        if not shape or shape[-1] != 4:
            raise RuntimeError(f"xarm6 abstract-hint seed expects observed 4D action space, got {shape!r}.")
...
def build_robot(env: Any, *, control_mode: str, robot_uid: str) -> GeneratedXArm6AbstractHintPullCubeRobot:
    """Factory used by ``real_runner --adapter-module``."""

    return GeneratedXArm6AbstractHintPullCubeRobot(env, robot_uid=robot_uid, control_mode=control_mode)
